...
- 问题一：他们的Megvii Face Recognition System经过训练后，在LFW数据集上达到了0.995的准确率。在真实场景测试中（Chinese ID （CHID）），该系统的假阳性率（![[公式]](https://www.zhihu.com/equation?tex=FP%3D10%5E%7B-5%7D+)）非常低。但是，真阳性率仅为0.66，没有达到真实场景应用要求。其中，年龄差异（包括intra-variation：同一个人，不同年龄照片；以及inter-variation：不同人，不同年龄照片）是影响模型准确率原因之一。而在该测试标准(CHID)下，人类表现的准确率大于0.90。
